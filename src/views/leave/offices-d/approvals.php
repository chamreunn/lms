...
<?php if (!empty($resign)): ?>
    ...
    <div class="row g-3">
        <?php foreach ($resign as $index => $item): ?>
            <div class="col-md-6 col-lg-3">
                <div class="card h-100 hover-shadow-lg overflow-hidden">
                    <div class="ribbon <?= $item['color'] ?>">
                        <?php if ($item['type'] == 'transfer'): ?>
                            <span>លិខិតផ្ទេរចេញ</span>
                        <?php elseif ($item['type'] == 'hold'): ?>
                            <span>លិខិតពួ្យរការងារ</span>
                        <?php elseif ($item['type'] == 'resign'): ?>
                            <span>លិខិតលាឈប់</span>
                        <?php else: ?>
                            <span>លិខិតចូលបម្រើការងារវិញ</span>
                        <?php endif; ?>
                    ...
                    <a href="#" class="text-decoration-none text-dark" data-bs-toggle="modal"
                        ...
                        <div class="card-body p-3 d-flex align-items-center">
                            <div class="avatar-container me-3">
                                <img class="avatar" style="object-fit: cover;" src="<?= $item['profile'] ?>" alt="User Profile">
                            </div>
                            <div class="info-container flex-grow-1">
                                <h5 class="mb-1 text-primary"><?= $item['user_name'] ?></h5>
                                <small class="text-muted fw-bold">
                                    <?= translateDateToKhmer($item['created_at'], 'j F Y h:i A') ?>
                                </small>
                            </div>
                        </div>
                    </a>
            ...
            <!-- Modal Structure -->
            <div class="modal modal-blur fade" id="detailModal<?= $item['id'] ?>" tabindex="-1"
                aria-labelledby="detailModalLabel<?= $item['id'] ?>" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-md">
                    <div class="modal-content">
                        <div class="modal-header <?= $item['color'] ?> text-light">
                            <h5 class="modal-title" id="detailModalLabel<?= $item['id'] ?>">លិខិតលាឈប់</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>

                        <!-- detail section  -->
                        <form action="/elms/action<?= $item['type'] ?>" method="POST">

                            <input type="hidden" name="resignId" value="<?= $item['id'] ?>">
                            ...
                            <div class="modal-body">
                                <div class="row mb-3 g-3">
                                    <div class="col-12 text-center">
                                        <!-- User Profile Picture -->
                                        <img src="<?= $item['profile'] ?>" alt="User Profile" class="avatar avatar-xl"
                                            style="object-fit: cover;">
                                    </div>
                                </div>
                                ...
                                <div class="bg-light p-3 rounded-3">
                                    <dl class="row g-1 mb-0">
                                        <dt class="col-sm-12 col-md-4"><strong>ឈ្មោះ:</strong></dt>
                                        <dd class="col-sm-12 col-md-8"><?= $item['workexperience'] ?></dd>

                                        <dt class="col-sm-12 col-md-4"><strong>មូលហេតុ:</strong></dt>
                                        <dd class="col-sm-12 col-md-8"><?= $item['reason'] ?></dd>
...
<?php if (!empty($transferouts)): ?>
    ...
    <div class="row g-3">
        <?php foreach ($transferouts as $index => $transferout): ?>
            <div class="col-md-6 col-lg-3">
                <div class="card h-100 hover-shadow-lg overflow-hidden">
                    <div class="ribbon <?= $transferout['color'] ?>">
                        <?php if ($transferout['type'] == 'transfer'): ?>
                            <span>លិខិតផ្ទេរចេញ</span>
                    ...
                    <!-- Trigger modal on click by adding data attributes -->
                    <a href="#" class="text-decoration-none text-dark" data-bs-toggle="modal"
                        data-bs-target="#detailModal<?= $transferout['id'] ?>">
                        <div class="card-body p-3 d-flex align-items-center">
                            <div class="avatar-container me-3">
                                <img class="avatar" style="object-fit: cover;" src="<?= $transferout['profile'] ?>" alt="User Profile">
                            </div>
                            <div class="info-container flex-grow-1">
            ...
            <!-- Modal Structure -->
            <div class="modal modal-blur fade" id="detailModal<?= $transferout['id'] ?>" tabindex="-1"
                aria-labelledby="detailModalLabel<?= $transferout['id'] ?>" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-md">
                    <div class="modal-content">
                        <div class="modal-header <?= $transferout['color'] ?> text-light">
                            <h5 class="modal-title" id="detailModalLabel<?= $transferout['id'] ?>">លិខិតលាឈប់</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>

                        <!-- detail section  -->
                        <form action="/elms/action<?= $transferout['type'] ?>" method="POST">

                            <input type="hidden" name="resignId" value="<?= $transferout['id'] ?>">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="bg-light p-3 rounded-3">
                                    <dl class="row g-1 mb-0">
                                        <dt class="col-sm-12 col-md-4"><strong>ឈ្មោះ:</strong></dt>
                                        <dd class="col-sm-12 col-md-8"><?= $transferout['workexperience'] ?></dd>
