...
class DepDepartmentModel
{
    ...
    {
        // Fetch all rejected leave requests from the database
        $stmt = $this->pdo->prepare('SELECT * FROM leave_requests 
     WHERE head_office = ?
     AND dhead_department = ?
     AND position IN (?, ?, ?, ?)
     AND department = ?
     AND user_id != ?');
        $stmt->execute(['Rejected', 'Rejected', 'មន្រ្តីលក្ខន្តិកៈ', 'ភ្នាក់ងាររដ្ឋបាល', 'អនុប្រធានការិយាល័យ', 'ប្រធានការិយាល័យ', $_SESSION['departmentName'], $approver_id]);
        $leaveRequests = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    private function updateRequestApproval($leave_request_id, $latestStatus)
    {
        ...
        // Update the leave request status
        $stmt = $this->pdo->prepare(
            'UPDATE ' . $this->table_name . ' SET head_department = ? WHERE id = ?'
        );
        $stmt->execute([$newStatus, $leave_request_id]);
    }
