...
class DepUnit1Model
{
    ...
    public function getRequestsByUserId($user_id, $token)
    {
    ...
    public function approvedCount()
    {
        ...
        // Execute the query with the session values
        $stmt->execute(['Approved', 'Rejected', 'Pending', 'Approved', 'មន្រ្តីលក្ខន្តិកៈ', 'ភ្នាក់ងាររដ្ឋបាល', 'អនុប្រធានការិយាល័យ', 'ប្រធានការិយាល័យ', 'អនុប្រធាននាយកដ្ឋាន', 'ប្រធាននាយកដ្ឋាន', 'នាយកដ្ឋានកិច្ចការទូទៅ', 'នាយកដ្ឋានសវនកម្មទី២', $_SESSION['user_id']]);
    ...
    public function rejectedCount()
    {
        ...
        // Execute the query with the session values
        $stmt->execute(['Approved', 'Rejected', 'Pending', 'Rejected', 'មន្រ្តីលក្ខន្តិកៈ', 'ភ្នាក់ងាររដ្ឋបាល', 'អនុប្រធានការិយាល័យ', 'ប្រធានការិយាល័យ', 'អនុប្រធាននាយកដ្ឋាន', 'ប្រធាននាយកដ្ឋាន', 'នាយកដ្ឋានកិច្ចការទូទៅ', 'នាយកដ្ឋានសវនកម្មទី២', $_SESSION['user_id']]);
    ...
    public function getapproved($user_id)
    {
        ...
        $stmt->execute(['Approved', 'Rejected', 'Pending', 'Approved', 'មន្រ្តីលក្ខន្តិកៈ', 'ភ្នាក់ងាររដ្ឋបាល', 'អនុប្រធានការិយាល័យ', 'ប្រធានការិយាល័យ', 'អនុប្រធាននាយកដ្ឋាន', 'ប្រធាននាយកដ្ឋាន', 'នាយកដ្ឋានកិច្ចការទូទៅ', 'នាយកដ្ឋានសវនកម្មទី២', $user_id]);
        $leaveRequests = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function getrejected($user_id)
    {
        ...
        $stmt->execute(['Approved', 'Rejected', 'Pending', 'Rejected', 'មន្រ្តីលក្ខន្តិកៈ', 'ភ្នាក់ងាររដ្ឋបាល', 'អនុប្រធានការិយាល័យ', 'ប្រធានការិយាល័យ', 'អនុប្រធាននាយកដ្ឋាន', 'ប្រធាននាយកដ្ឋាន', 'នាយកដ្ឋានកិច្ចការទូទៅ', 'នាយកដ្ឋានសវនកម្មទី២', $user_id]);
        $leaveRequests = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function leaveUserApproved($token)
    {
        ...
        // Define multiple departments to check against the session
        $sessionDepartments = ["នាយកដ្ឋានកិច្ចការទូទៅ", "នាយកដ្ឋានសវនកម្មទី២"];

        // Query to get leave requests that are approved and overlap with today's date
        $stmt = $this->pdo->prepare(
            'SELECT lr.id as leave_request_id, lr.user_id, lr.start_date, lr.end_date, lr.num_date,
                lr.department, lr.status
         FROM leave_requests lr
         WHERE ? BETWEEN lr.start_date AND lr.end_date
           AND lr.status = ?
           AND lr.department IN (' . implode(',', array_fill(0, count($sessionDepartments), '?')) . ')'
        );

        // Prepare query parameters (today's date, approved status, and department names)
        $params = array_merge([$today, 'Approved'], $sessionDepartments);
        $stmt->execute($params);
        $leaveRequests = $stmt->fetchAll();
    ...
    {
        $departmentNames = ['នាយកដ្ឋានកិច្ចការទូទៅ', 'នាយកដ្ឋានសវនកម្មទី២']; // Correct department names
        $today = date('Y-m-d');
        $leadersOnLeave = [];

        // Prepare placeholders for department names in the query
        ...
        FROM leave_requests lr
        JOIN leave_types lt ON lr.leave_type_id = lt.id
        WHERE lr.department IN ($placeholders) 
        AND lr.start_date <= ? 
        AND lr.end_date >= ? 
        AND lr.status = 'Approved'
    ");

        // Merge department names and today's date parameters
        $params = array_merge($departmentNames, [$today, $today]);
            ...
            if ($leaderResponse['http_code'] === 200 && isset($leaderResponse['data'])) {
                ...
                $leadersOnLeave[] = [
                    ...
                    'leave_request' => [
                        ...
                        'leave_type' => $leaveRequest['leave_type_name'], // Add leave type name
                    ],
                ];
            } else {
                // Handle any API errors or unexpected responses here
    ...
    public function delegateManager($holdRequestModel, $userModel, $hold_id, $user_id, $action)
    {
        $levels = ['getEmailLeaderHUApi'];
    ...
    public function insertManagerStatusAndUpdateApprover($hold_id, $approver_id, $status, $action = null)
    {
        // Debugging checks
        if (is_array($hold_id)) {
        ...
        try {
            // Check if the manager is on leave or mission based on the status
            if (in_array($status, ['leave', 'mission'])) {
                // Update only the `status` column in the `holds` table
                $updateSql = "UPDATE holds SET status = :status WHERE id = :hold_id";
                $updateStmt = $this->pdo->prepare($updateSql);
                $updateStmt->execute([
                    ':status' => $action, // Use the provided action for the status
                    ':hold_id' => $hold_id,
                ]);
            } else {
                // Normal case: Insert into holds_approvals and update `approver_id`
