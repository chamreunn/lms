...
class DepUnit2Model
{
    ...
    {
        // Define the department names to filter leave requests for
        $departmentNames = ['នាយកដ្ឋានសវនកម្មទី១']; // You can add more department names if needed
        $today = date('Y-m-d');
        $leadersOnLeave = [];
        $currentUserId = $_SESSION['user_id']; // Fetch the current user ID from the session

        // Prepare placeholders for the departments in the SQL query
        $placeholders = implode(',', array_fill(0, count($departmentNames), '?'));

        // Prepare the SQL query to get leave requests joined with leave types
        $stmt = $this->pdo->prepare("
        SELECT lr.*, lt.name AS leave_type_name
        FROM leave_requests lr
        JOIN leave_types lt ON lr.leave_type_id = lt.id
        WHERE (lr.department IN ($placeholders) OR lr.user_id = ?)
        AND lr.start_date <= ? 
        AND lr.end_date >= ? 
        AND lr.status = 'Approved'
    ");

        // Prepare the parameters for the query (departments, user ID, and the current date)
        $params = array_merge($departmentNames, [$currentUserId, $today, $today]);
