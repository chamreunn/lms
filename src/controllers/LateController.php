...
class LateController
{
    ...
    public function createLateIn($date, $time, $reason)
    {
        ...
        try {
            ...
            $_SESSION['success'] = [
                'title' => "សំណើចូលយឺត",
                'message' => "អ្នកបានយឺតចំនួន {$lateMinutes} នាទី។ សំណើបានបញ្ជូនទៅកាន់ " . $userDoffice['dkhmer_name'] . " សូមមេត្តារង់ចាំ។ សូមអរគុណ។"
            ];
            header("Location: /elms/overtimein");
            exit();
        } catch (Exception $e) {
    ...
    private function sendEmailNotification($adminEmail, $date, $time, $lateMinutes, $reason)
    {
        ...
        try {
            ...
            // Format dates
            $start_date_formatted = (new DateTime($date))->format('j F, Y');
            $end_date_formatted = (new DateTime($time))->format('j F, Y');
